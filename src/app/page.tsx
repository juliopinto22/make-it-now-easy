...
export default function App() {
  ...
  const otimizacoesCMD = [
    ...
    { id:15, nome:'Otimizar desempenho do sistema', cmd:'sysdm.cpl → Avançado → Desempenho → Ajustar para obter melhor desempenho', perigo:'alto' },
    ...
    { id:18, nome:'Definir número de processadores na inicialização', cmd:'bcdedit /set {current} numproc %NUMBER_OF_PROCESSORS%', perigo:'alto' },
    { id:19, nome:'Desativar limite de memória', cmd:'bcdedit /deletevalue {current} truncatememory', perigo:'alto' },
    { id:20, nome:'Limpar log de visualização de eventos', cmd:'for /f "tokens=*" %1 in (\'wevtutil el\') do wevtutil cl "%1"', perigo:'alto' },
    ...
    { id:32, nome:'Desativar efeitos visuais', cmd:'reg add "HKCU\\Control Panel\\Desktop" /v VisualFXSetting /t REG_DWORD /d 2 /f', perigo:'médio' },
    ...
    { id:34, nome:'Desativar throttling de CPU', cmd:'reg add "HKLM\\System\\CurrentControlSet\\Services\\Power\\Throttling" /v NoThrottling /t REG_DWORD /d 1 /f', perigo:'alto' },
    { id:35, nome:'Otimizar agendador de rede', cmd:'netsh int tcp set global congestionprovider=ctcp', perigo:'alto' },
    { id:36, nome:'Ativar receção de janela automática', cmd:'netsh int tcp set global autotuninglevel=normal', perigo:'médio' },
    ...
    { id:40, nome:'Desativar tempo limite de conexão', cmd:'reg add "HKLM\\System\\CurrentControlSet\\Services\\Tcpip\\Parameters" /v TcpTimedWaitDelay /t REG_DWORD /d 30 /f', perigo:'alto' },
    ...
    { id:42, nome:'Aumentar tamanho do buffer TCP', cmd:'netsh int tcp set global rss=enabled', perigo:'médio' },
    { id:43, nome:'Ativar TCP Fast Open', cmd:'netsh int tcp set global tcp1323opts=enabled', perigo:'alto' },
    { id:44, nome:'Limpar cache de fonte', cmd:'del /f /s /q "%LocalAppData%\\Microsoft\\Windows\\Fonts\\*"', perigo:'alto' },
    { id:45, nome:'Reduzir tempo de espera de menu', cmd:'reg add "HKCU\\Control Panel\\Desktop" /v MenuShowDelay /t REG_SZ /d 20 /f', perigo:'médio' },
    ...
    { id:47, nome:'Desativar atalhos de ícone', cmd:'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer" /v Link /t REG_BINARY /d 00000000 /f', perigo:'médio' },
    ...
    { id:49, nome:'Desativar atualização em segundo plano', cmd:'reg add "HKCU\\Control Panel\\Desktop" /v PaintDesktopVersion /t REG_DWORD /d 0 /f', perigo:'baixo' },
    ...
    { id:53, nome:'Desativar aplicativos em segundo plano', cmd:'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\BackgroundAccessApplications" /v GlobalDisabled /t REG_DWORD /d 1 /f', perigo:'alto' },
    ...
    { id:55, nome:'Desativar lembrete de reinicialização', cmd:'reg add "HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Installer" /v DisableRollback /t REG_DWORD /d 1 /f', perigo:'alto' },
    { id:56, nome:'Otimizar memória — Limpeza RAM', cmd:'Rundll32.exe advapi32.dll,ProcessIdleTasks', perigo:'médio' },
    { id:57, nome:'Desfragmentar disco (SSD não recomendado!)', cmd:'defrag C: /O', perigo:'alto' },
    { id:58, nome:'Verificar arquivos do sistema', cmd:'sfc /scannow', perigo:'alto' },
    { id:59, nome:'Verificar imagem do sistema', cmd:'DISM /Online /Cleanup-Image /RestoreHealth', perigo:'alto' },
    { id:60, nome:'Limpar atualizações antigas', cmd:'dism /online /cleanup-image /spsuperseded', perigo:'alto' },
    { id:61, nome:'Redefinir rede', cmd:'netsh winsock reset', perigo:'alto' },
    ...
    { id:64, nome:'Remover rede pré-configurada', cmd:'netsh wlan delete profile name=*', perigo:'médio' },
    ...
    { id:66, nome:'Desativar gerenciamento de energia PCIe', cmd:'reg add "HKLM\\System\\CurrentControlSet\\Services\\pci" /v ExpressPcieAspm /t REG_DWORD /d 0 /f', perigo:'alto' },
    { id:67, nome:'Desativar gerenciamento de link', cmd:'reg add "HKLM\\System\\CurrentControlSet\\Control\\Class\\{4d36e972-e325-11ce-bfc1-08002be10318}\\0000" /v *EEE /t REG_SZ /d 0 /f', perigo:'alto' },
    { id:68, nome:'Aumentar limite de CPU em primeiro plano', cmd:'reg add "HKLM\\System\\CurrentControlSet\\Control\\Session Manager" /v LargeSystemCache /t REG_DWORD /d 0 /f', perigo:'alto' },
    { id:69, nome:'Desativar página de memória de kernel', cmd:'reg add "HKLM\\System\\CurrentControlSet\\Control\\Session Manager\\Memory Management" /v DisablePagingExecutive /t REG_DWORD /d 1 /f', perigo:'alto' },
    { id:70, nome:'Limpar tudo e finalizar', cmd:'cleanmgr /sagerun:1', perigo:'médio' },
  ];

  // === 10 FAST FLAGS ROBLOX — 5 FPS + 5 GRÁFICOS ===
  const fastFlags = [
    { id:1, nome:'FPS Ilimitado', flag:'FramerateLimit = 0', tipo:'fps' },
    { id:2, nome:'Otimização de Render', flag:'RenderingMode = "Automatic"', tipo:'fps' },
    { id:3, nome:'Física Leve', flag:'PhysicsEnvironment = "Performance"', tipo:'fps' },
    { id:4, nome:'Sem Sombra Dinâmica', flag:'DynamicShadows = false', tipo:'fps' },
    { id:5, nome:'Qualidade de Partícula Baixa', flag:'ParticleQuality = 1', tipo:'fps' },
    { id:6, nome:'Textura HD', flag:'TextureQuality = "High"', tipo:'graficos' },
    { id:7, nome:'Sombras Suaves', flag:'ShadowQuality = 3', tipo:'graficos' },
    { id:8, nome:'Iluminação Aprimorada', flag:'GraphicsMode = "Automatic"', tipo:'graficos' },
    { id:9, nome:'Reflexos Ativados', flag:'RefractionQuality = 2', tipo:'graficos' },
    { id:10, nome:'Efeitos de Luz', flag:'BloomEffect = true', tipo:'graficos' },
  ];
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h1 style={{ fontSize: 42, margin: 0, letterSpacing: 6, color: '#cc0000' }}>KANEKI</h1>
          <p style={{ color: '#888', letterSpacing: 4, fontSize: 13, marginTop: 5, marginBottom: 30 }}>OPTIMIZER</p>
          <div style={{ borderTop: '1px solid #330000', marginBottom: 25 }}></div>
          <h3 style={{ fontWeight: 400, color: '#ddd', marginBottom: 25 }}>Você é um robô?</h3>
          <p style={{ color: '#777', fontSize: 13, marginBottom: 30 }}>Confirme que você é humano para acessar o painel.</p>
          ...
          {!carregando ? (
            <button 
              ...
              onMouseOver={(e) => { e.currentTarget.style.boxShadow = '0 0 20px #8b0000'; }}
              onMouseOut={(e) => { e.currentTarget.style.boxShadow = 'none'; }}
            >
              ✅ Não, sou humano
            </button>
          ) : (
            <div style={{ color: '#cc0000', padding: 15 }}>Verificando... 🔄</div>
          )}
          
          <p style={{ fontSize: 11, color: '#555', marginTop: 25, lineHeight: 1.5 }}>
            Ao acessar, você concorda que os comandos são de uso por sua conta e risco.<br/>
            Projeto inspirado em Tokyo Ghoul — Sui Ishida.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div style={{
      ...
      <div style={{ marginLeft: 240, flex: 1, padding: '30px 40px', maxWidth: 900 }}>
          ...
          <>
            ...
            <div style={{
              ...
              <p style={{ color: '#ddd', fontSize: 13, margin: 0, lineHeight: 1.6 }}>
                <span style={{ color:'#22c55e' }}>● Verde:</span> Seguro<br/>
                <span style={{ color:'#eab308' }}>● Amarelo:</span> Cuidado — altera configurações de rede/serviços<br/>
                <span style={{ color:'#ef4444' }}>● Vermelho:</span> PERIGOSO — altera registro e sistema. Use com conhecimento!
              </p>
            </div>
        ...
        {abaAtiva === 'premium' && (
          <>
            <h2 style={{ color: '#ffcc00', marginBottom: 25 }}>👑 Área Premium</h2>
            <div style={{
              maxWidth: 380,
              margin: '0 auto',
              textAlign: 'center',
              ...
              <h3 style={{ color: '#ffcc00', margin: '0 0 20px 0', letterSpacing: 3, fontSize: 16 }}>PAGAMENTO VIA PIX</h3>
              
              {pixChave && (
                <img 
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=220x220&color=8B0000&bgcolor=0F0505&data=${encodeURIComponent(pixChave)}`}
                  alt="QR Code PIX"
                  style={{ 
                    width: 220, 
                    height: 220, 
              ...
              <p style={{ color: '#cc9999', marginTop: 18, fontSize: 14 }}>💳 Escaneie o QR Code</p>
